...
// let count = 0;
// let hearts;
let inventoryDisplay: Phaser.GameObjects.Text;
...
export default class GameUI extends Phaser.Scene {
  ...
  create({ data, player }: { data: any; player: any }) {
    ...
    if (data.inventory) {
      inventoryDisplay = this.add.text(
        35,
        40,
        ": " + data.inventory.length + "/36",
        { fontSize: 25, fontFamily: "VT323" }
      );
    } else {
      inventoryDisplay = this.add.text(35, 40, ": " + 0, {
        fontSize: 25,
        fontFamily: "VT323",
      });
    }
